...
const Sidebar = () => {
  ...
  return (
    <motion.div whileInView={{ x: [-235, 0] }} transition={{ duration: 0.5 }} className="fixed  flex z-40 w-[235px] pl-[6px] pt-[6px] bg-dark h-auto overflow-y-auto flex-col">
      ...
      <Box sx={{ borderBottom: 1, borderBottomColor: "rgba(255, 255, 255, 0.2)" }}>
        <Box sx={{ display: "flex", flexDirection: "column", borderBottom: "2px", borderBottomColor: "white" }}>
          {first.map((item) => (
            <button key={item.name} className="flex space-x-2 p-2 md:p-3 rounded-lg hover:bg-white/20 items-center">
